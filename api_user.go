...
/*
UserFollowing 获取当前用户的关注列表，或者判断是否关注某个特定用户。
...
// Execute executes the request
//  @return BooleanApiResponse
func (a *UserAPIService) UserFollowingExecute(r ApiUserFollowingRequest) (*BooleanApiResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
...
/*
UserOAuthAccountBind 外部账号 绑定，如果已存在绑定则直接返回成功
...
*/
func (a *UserAPIService) UserProfileById(ctx context.Context, userId int64, appKey string) ApiUserProfileByIdRequest {
	return ApiUserProfileByIdRequest{
		ApiService: a,
		ctx: ctx,
		userId: userId,
...
func (a *UserAPIService) UserProfileByIdExecute(r ApiUserProfileByIdRequest) (*GetUserProfileResultApiResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "UserAPIService.UserProfileById")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/User/{appKey}/Profile/{userId}"
	localVarPath = strings.Replace(localVarPath, "{"+"userId"+"}", url.PathEscape(parameterValueToString(r.userId, "userId")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"appKey"+"}", url.PathEscape(parameterValueToString(r.appKey, "appKey")), -1)
