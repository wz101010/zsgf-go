...
type ApiStorageBatchUpdateRequest struct {
	ctx context.Context
	ApiService *StorageAPIService
	table string
	filter *string
...
// 查询条件
func (r ApiStorageBatchUpdateRequest) Filter(filter string) ApiStorageBatchUpdateRequest {
	r.filter = &filter
	return r
}

// 更新的字段
func (r ApiStorageBatchUpdateRequest) RequestBody(requestBody []interface{}) ApiStorageBatchUpdateRequest {
	r.requestBody = &requestBody
	return r
}

// 是否为全量替换，默认为 false
func (r ApiStorageBatchUpdateRequest) Replace(replace bool) ApiStorageBatchUpdateRequest {
	r.replace = &replace
	return r
}

func (r ApiStorageBatchUpdateRequest) Execute() (*BooleanApiResponse, *http.Response, error) {
	return r.ApiService.StorageBatchUpdateExecute(r)
}

/*
StorageBatchUpdate 批量更新指定条件的数据
...
批量更新指定表中符合条件的文档，可以动态指定查询条件和更新字段
 ...
 @param table 表名称
 @param appKey
 @return ApiStorageBatchUpdateRequest
*/
func (a *StorageAPIService) StorageBatchUpdate(ctx context.Context, table string, appKey string) ApiStorageBatchUpdateRequest {
	return ApiStorageBatchUpdateRequest{
		ApiService: a,
		ctx: ctx,
		table: table,
...
func (a *StorageAPIService) StorageBatchUpdateExecute(r ApiStorageBatchUpdateRequest) (*BooleanApiResponse, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "StorageAPIService.StorageBatchUpdate")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/Storage/{appKey}/{table}/batch-update"
	localVarPath = strings.Replace(localVarPath, "{"+"table"+"}", url.PathEscape(parameterValueToString(r.table, "table")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"appKey"+"}", url.PathEscape(parameterValueToString(r.appKey, "appKey")), -1)
